...
export function CsvImportDialog({ open, onOpenChange, onImport, existingContacts }: CsvImportDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) reset(); onOpenChange(v); }}>
      <DialogContent className="bg-card border-border max-w-lg w-[calc(100vw-2rem)] overflow-hidden w-[calc(100vw-2rem)] overflow-hidden">
        <DialogHeader>
        ...
        {!preview ? (
          <div className="space-y-4 pt-2">
            ...
            >
              ...
              <p className="text-xs text-muted-foregroun MAP ��援一般 CSV 及組織圖ＭＡＰ格式（自動偵測）</p>
            </div>
