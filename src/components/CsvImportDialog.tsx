...
export function CsvImportDialog({ open, onOpenChange, onImport, existingContacts }: CsvImportDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) reset(); onOpenChange(v); }}>
      <DialogContent
        className="max-w-lg w-[calc(100vw-2rem)] overflow-hidden p-0 border-0 bg-transparent !top-[calc(env(safe-area-inset-top)+2dvh)] [&>button]:!top-[calc(env(safe-area-inset-top)+0.5rem)] !translate-y-0 sm:!top-[50%] sm:!translate-y-[-50%] [&>button]:z-30 [&>button]:bg-black/50 [&>button]:rounded-full [&>button]:p-1"
      ...
      >
        <div className="relative overflow-hidden rounded-lg h-full">
          ...
          <div className="relative z-10 p-4 sm:p-6 pt-10 pb-8 overflow-y-auto overscroll-contain" style={{ maxHeight: "calc(96dvh - env(safe-area-inset-top))", WebkitOverflowScrolling: "touch" }}>
            <DialogHeader>
