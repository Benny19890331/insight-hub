            ...
            <>
                ...
                <button
                  ...
                  onClick={() => {
                    lovable.auth.signInWithOAuth("google", {
                      ...
                      if (result.error) {
                        toast.error(isLogin ? "Google 登入失敗，請稍後再試" : "Google 註冊失敗，請稍後再試");
                        console.error("Google OAuth error:", result.error);
                      }
                      if (result.redirected) return;
                      setLoading(false);
                    }).catch((err) => {
                      toast.error(isLogin ? "Google 登入失敗" : "Google 註冊失敗");
                      console.error(err);
                      setLoading(false);
                    });
                  }}
              ...
              <p className={`text-center text-xs ${t.authSubtext}`}>
                ...
                <button
                  ...
                  className={`${t.authLink} ml-1 underline-offset-2 hover:underline`}
                >
                  {isLogin ? "立即註冊" : "回到登入"}
                </button>
              </p>
