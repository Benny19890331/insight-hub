...
const activeLevel = (u: AdminUser): { label: string; color: string } => {
  ...
  const recentScore = (u.monthlyTrend || []).slice(-2).reduce((a, b) => a + b, 0);

  if (recentScore >= 40 || lastSignInDays <= 2) return { label: "高活躍", color: "text-green-400 border-green-500/40 bg-green-500/10" };
  if (recentScore >= 10 || lastSignInDays <= 7) return { label: "中活躍", color: "text-yellow-400 border-yellow-500/40 bg-yellow-500/10" };
  if (recentScore > 0 || lastSignInDays <= 30) return { label: "低活躍", color: "text-blue-400 border-blue-500/40 bg-blue-500/10" };
