...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    // Also check if session already exists (user may have already been authenticated via the link)
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="flex h-32 w-32 items-center justify-center" style={{ filter: `drop-shadow(0 0 24px ${t.titleGlow}) drop-shadow(0 4px 8px rgba(0,0,0,0.3))`, perspective: "200px" }}>
            <div className="relative" style={{ transform: "rotateX(12deg) rotateY(-8deg)", transformStyle: "preserve-3d" }}>
              <Infinity className="h-28 w-28" style={{ stroke: "url(#resetMetalGrad)", strokeWidth: 2.2, filter: "drop-shadow(2px 4px 3px rgba(0,0,0,0.4)) drop-shadow(-1px -1px 0px rgba(255,255,255,0.15))" }} />
            </div>
            <svg width="0" height="0">
              <defs>
                <linearGradient id="resetMetalGrad" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <h1 className="text-2xl font-semibold">
            ...
            <span className={`ml-2 font-normal ${t.authCardText}`}>重設密碼</span>
          </h1>
        </div>
