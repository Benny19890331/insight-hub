...
serve(async (req) => {
  ...
  try {
    ...
    const probeText = [
      contact.background, contact.notes,
      Array.isArray(contact.product_tags) ? contact.product_tags.join(" ") : contact.product_tags,
      Array.isArray(contact.statuses) ? contact.statuses.join(" ") : "",
      (interactionsRes.data ?? []).map((i: any) => i?.summary ?? "").join(" "),
    ].filter(Boolean).join(" ");
...
- summary: **重點條列式**摘要（不是段落文章）。每一項一行，使用「• 項目名稱：內容」格式，項目之間用單一換行符號 \\n 分隔（**不要連續多個換行、不要在結尾加多餘的 \\n 或空的 •**）。請依照下列固定順序輸出，但**只列出有實質內容、且資料中真的提到或可合理推論的項目**，其餘**完全省略**（不要寫「資料不足」、「無」、「未知」、「待補」，也不要保留空白項目）：
  ...
  • 想要找哪位領袖借力（從互動紀錄推測對方可能信任或想接近的領袖對象）
  • 需求（目前最可能的需求或痛點）
  • 關係（與我們的關係深淺、認識來源）
  • 邀約動機（最能打動對方的切入點或誘因，一句話）
