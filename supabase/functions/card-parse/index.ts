...
重要：原診斷是寫給「當事人本人」看的（第二人稱「你」），但我們是顧問在做筆記，請一律用第三人稱描述受測者（用姓名或「對方」），不要出現「你」「我」。
請你輸出嚴格 JSON（無 markdown），欄位如下：
- name: 受測者姓名（若原文已有 👤 受測者就用該值）
- type: 人格類型短句（如「領航為主」「穩健執行」），若原文有 🧬 類型就直接沿用
- state: 內在狀態短句，改寫成第三人稱（例：原文「我想突破事業」→「想突破事業」或「他想突破事業」）
- heat: "cold" | "warm" | "hot" | "loyal"，依內在狀態語氣推斷，預設 "warm"
- summary: 80 字內中文，2-3 句以第三人稱濃縮其人格特質與行動偏好（從 10 題作答歸納，不要逐題複述，不要用「你」）

範例：{"name":"小胖","type":"領航為主","state":"想突破事業","heat":"hot","summary":"小胖偏好抓趨勢、先出手再優化；重視 SOP 與未來藍圖；不耐慢節奏，遇卡關傾向換路找新風口。"}
